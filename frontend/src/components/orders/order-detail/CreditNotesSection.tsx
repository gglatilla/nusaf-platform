...
export function CreditNotesSection({
  ...
  // Customers only see ISSUED credit notes
  ...
  const handleVoid = async (id: string) => {
    ...
    await voidCN.mutateAsync({ id, reason: voidReason });
    setVoidingId(null);
    setVoidReason('');
  };
        ...
        {filtered.map((cn) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() =>
                  download.mutate({ id: cn.id, creditNoteNumber: cn.creditNoteNumber })
                }
              ...
              {canVoid && cn.status === 'ISSUED' && (
                <button
                  onClick={() => setVoidingId(cn.id)}
          ...
          <div className="flex min-h-screen items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <p className="text-sm text-slate-600 mb-4">
                This will mark the credit note as voided. This action cannot be undone.
              </p>
              <textarea
                value={voidReason}
              ...
              <div className="flex justify-end gap-3 mt-4">
                ...
                <button
                  ...
                  disabled={!voidReason.trim() || voidCN.isPending}
                  className="px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 disabled:opacity-50"
                >
                  {voidCN.isPending ? 'Voiding...' : 'Void Credit Note'}
                </button>
              </div>
            </div>
          </div>
